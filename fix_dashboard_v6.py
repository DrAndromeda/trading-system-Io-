...
new_func = '''def update_dashboard(state, latest):
    ...
    CAP_PER_POS = CAPITAL / MAX_POSITIONS  # how much capital per position
    ...
    for s in latest:
        if s["signal"] == "LONG":
            badge, cls = '<div class="sig-badge long">🟢 LONG</div>', "sig-long"
        elif s["signal"] == "WEAK_LONG":
            badge, cls = '<div class="sig-badge weak">🟡 WAIT</div>', "sig-weak"
        elif s["signal"] == "OVERBOUGHT":
            badge, cls = '<div class="sig-badge short">🔴 OVERBOUGHT</div>', "sig-short"
        elif s["signal"] == "OVERSOLD":
            badge, cls = '<div class="sig-badge short">🔴 OVERSOLD</div>', "sig-short"
        else:
            badge, cls = '<div class="sig-badge wait">⚪ WAIT</div>', "sig-wait"

        if s["confidence"] >= 80:
            strength, sc = "STRONG", "#3fb950"
        elif s["confidence"] >= 70:
            strength, sc = "MEDIUM", "#d29922"
        else:
            strength, sc = "WEAK", "#8b949e"

        size_usd = s["size"] * s["entry"]
        risk_usd = size_usd * (s["entry"] - s["stop"]) / s["entry"]
        reward_usd = size_usd * (s["take"] - s["entry"]) / s["entry"]
        leverage = size_usd / CAP_PER_POS
        ...
        cards += f\'\'\'<div class="sig-card {cls}">
          ...
          <div class="sig-strength">
            <span style="color:{sc};font-weight:700">{strength}</span>
            <span style="color:{sc}">{s["confidence"]}%</span>
          </div>
          ...
          <div class="sig-info">
            ...
            <div class="sig-row"><span>Плечо</span><span class="warn">{leverage:.1f}x</span></div>
    ...
    for sym, p_data in positions.items():
        ...
        pos_rows += f\'\'\'<tr>
          ...
          <td class="warn"><b>{leverage:.1f}x</b></td>
...
<b>STRONG</b> 80%+ | <b>MEDIUM</b> 70–79% | <b>Плечо</b> = notional / ${CAP_PER_POS:,.0f} (капитал на позицию) |
<b>Тренд 15m/1h/4h/1d</b> — все 4 должны быть 🟢 для LONG
</div>
...
<table>
<tr>
  <th>Монета</th><th>Тип</th><th>Вход</th><th>Сейчас</th><th>PnL %</th><th>PnL $</th>
  <th>Notional</th><th>Плечо</th><th>Stop</th><th>Take</th><th>Прогресс</th><th>Trails</th>
</tr>
{pos_rows}
</table>
...
<div class="tf-tabs" id="tfTabs">
  <div class="tf-tab active" data-tf="15m">15 минут</div>
  <div class="tf-tab" data-tf="1h">1 час</div>
  <div class="tf-tab" data-tf="4h">4 часа</div>
  <div class="tf-tab" data-tf="1d">1 день</div>
</div>
...
document.querySelectorAll('.tf-tab').forEach(t => {{
  t.onclick = () => {{ currentTF = t.dataset.tf; renderChart(); }};
}});

const tabs = document.getElementById('tabs');
Object.keys(CHARTS).forEach((sym, i) => {{
  const t = document.createElement('div');
  t.className = 'chart-tab';
  t.dataset.sym = sym;
...
if (Object.keys(CHARTS).length > 0) {{
  currentSym = Object.keys(CHARTS)[0];
  renderChart();
}}
</script>
</body></html>"""
